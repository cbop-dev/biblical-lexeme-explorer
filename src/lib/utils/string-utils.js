...
export function capitalize(string){
    return string ? 
            string[0].toLocaleUpperCase() + (string.length > 1 ? string.substring(1) : '')
            : '';
}
...
export function uncapitalize(string){
    return string[0].toLocaleLowerCase() + (string.length > 1 ? string.substring(1) : '');
}
 ...
 * @returns {boolean}
 */
export function equal(str1,str2,ignoreCase=false){
    if(ignoreCase) {
